...
When a decision materially changes the work, the root model can call `ask_user` with related multiple-choice questions. The kernel persists one bounded `input_requested` event with at most 60 questions, and the maximum batch is covered at clipped and normal viewport heights. The TUI gives the question controller the complete Vaxis frame, using the same quiet settings-style surface as the command palette/settings overlay: one horizontal row per visible question, Enter select, Space toggle, Up/Down question focus, Left/Right option focus, inline `f / Other`, and an explicit review/submit state. The full-frame boundary prevents transcript, reasoning-dock, status-bar, and footer geometry from colliding with a long questionnaire. Multi-select questions remain on their row until review, matching the reference dialog interaction. The root normal path and `orchestrate`, `build`, `align`, and `plan` share this controller. Both idle and streaming key paths use one recovery boundary: controller or `input/respond` errors keep the panel active instead of unwinding the TUI. The frame projection rejects invalid or oversized envelopes, rejects invalid UTF-8/control text, truncates by terminal cell width, uses static display keys while preserving response ids, and guards clipped viewports. Malformed question payloads become one bounded system message plus run cancellation instead of a TUI crash. `input/respond` remains the only resolution path; no question poller, second status bus, transcript copy, or child input loop exists. Headless child profiles do not receive the interactive tool and continue autonomously or report the unavailable capability.
