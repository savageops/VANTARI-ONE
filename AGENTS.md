...
- The TUI composer owns a transient registry-backed command palette above the input. It matches the first single token by prefix, accepts bare names while retaining slash compatibility, supports Up/Down/Escape/Tab/Enter, and disappears for prose or no matches. The popup is bounded and uses the existing command metadata; do not add a second command registry, async scanner, or slash-only requirement.
