...
- Child group rows show `Agents completed/total` with `○` for queued/running and `◉` for complete; child rows use the same state language and render the latest canonical summary as a bounded quoted suffix. Known typed phases and elapsed snapshots may precede that summary; later tool/terminal events update the keyed state without replacing it. Do not render the removed `waiting on N` filler, expose `tool_completed` as the child's visible summary, or add a chat-bubble event, poller, second ledger, or TUI-owned summary bus.
