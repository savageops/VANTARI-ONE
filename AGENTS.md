...
- `clients/models_view.zig::State` is the model-selection overlay: one flat catalog from `models/list-all` (every credentialed provider, active group first, per-group unreachable degradation), grouped under provider headers with the active model marked, type-to-filter across model and provider ids, and Enter committing through `providers/set-model` with the model's source provider — a cross-provider pick selects the provider in the same action, the TUI never manages active-provider state itself. The palette opens it when the composer token is exactly `model` followed by Space, and bare `model` opens it instead of the settings Models tab; commit flips the footer model and appends one bounded system row. Per-agent model overrides remain the Settings → Models assign layer plus `agents/configure`; do not add a second catalog owner, client-side provider switching, or a poller.
