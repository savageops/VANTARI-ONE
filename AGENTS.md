...
- `clients/question_view.zig::State` is the one question projection: render bounded batches as settings-style horizontal rows with clamped question/option focus and an explicit review/submit state. `orchestrate` and `align` share this controller. Malformed `input_requested` data is recoverable client input; report it once and cancel the waiting run without unwinding the TUI event loop.
