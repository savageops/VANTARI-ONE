...
- `core/tools/builtin/repair_candidate.zig` is the sole proposal owner for Move 74. It resolves through `fsutil.resolveWithAccessMode`, requires read-ledger inspection and an existing target, captures the before hash, hashes the operation/path/patch descriptor, appends `var1.repair_candidate.v1` to the existing event spine, and returns `RepairBaselineConflict` on source drift. Every candidate reports `mutation_allowed:false`; later approval/application uses the existing reviewed write tools. Do not add a patcher, approval bus, or repair ledger.
- `stdio_rpc.zig::handleRepairCandidateApprove` and `evaluation/events.zig::appendRepairCandidateApprovalEvent` are the sole Move 75 approval owners. The operator-only `repair/approve` socket binds candidate event sequence/id, exact stored patch hash, and expected source baseline; the event owner rechecks the current baseline, appends `var1.repair_candidate_approval.v1`, and returns the existing approval sequence for the same approval identity. It is evidence only; Move 76 uses existing reviewed write tools. Do not expose approval as a model tool or add a broker, queue, patcher, or second ledger.
