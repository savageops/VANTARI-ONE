...
- `stdio_rpc.zig::handleRepairRerun` and `loop.zig::RunOptions.expected_replay_*` own Move 77 treatment admission. The handler validates the source receipt/applied edge, creates the fresh linked child, and reuses `handleSessionSend`; the executor compares exact input/config hashes before context/provider work. Persist only compact relationship receipts and hashes. Do not substitute a prompt, copy the parent transcript, add a replay provider path, or turn an interrupted start into an implicit retry; Moves 78–80 own evaluation, rollback, and cold-start promotion.
