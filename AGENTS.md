...
- `stdio_rpc.zig::handleRepairRerun` and `loop.zig::RunOptions.expected_replay_*` own Moves 77–78 treatment admission/evaluation. The handler validates the source receipt/applied edge, creates the fresh linked child, reuses `handleSessionSend`, and appends one bounded evaluator receipt to the existing source spine; the executor compares exact input/config hashes before context/provider work. Persist only compact relationship/evaluation metrics and hashes. Do not substitute a prompt, copy the parent transcript, add a replay provider path, add an evaluator worker/ledger, or turn an interrupted start into an implicit retry; Moves 79–80 own rollback and cold-start promotion.
