...
- `core/prompts/builder.zig::PromptMode` owns the session-local `orchestrate -> build -> align -> plan` prompt lens and root orchestration posture. Shift+Tab is the fast TUI control; the next `session/send` carries its exact lower-case label, omission defaults to `orchestrate`, and unknown labels fail before session/provider execution. `orchestrate` exposes only agent/collaboration/interactive tools to a root run and requires the current eligibility snapshot for agent actions; `build`, `align`, and `plan` retain the normal root catalog. `agent_routes.prompt_modes` may select a provider/model and turn budget for one mode through the existing route shape; explicit `session/send` overrides win. Child capability profiles are unchanged, and no persistent `agents.orchestrator_only` policy exists.
