...
| Failure evidence | failed and timed-out terminal rows carry one bounded `var1.failure_receipt.v1`; completed and cancelled rows carry none. |
| Replay receipt | each admitted root turn appends one `var1.repair_receipt.v1` before context/provider dispatch; original input/model remain exact, config/tool/catalog/source/environment are hashed, and raw snapshots stay out of `events.jsonl`. |
| Replay diagnosis | failed and timed-out `turn_terminal.failure` payloads carry one bounded `var1.repair_diagnosis.v1` with a fixed invariant, deterministic ID, and `session_started.seq` → `turn_terminal.seq` span; completed/cancelled payloads carry none. |
| Repair candidate | proposal-only `repair_candidate` appends one `var1.repair_candidate.v1` receipt with operation/path/patch hashes and expected/current source baselines; drift returns a typed conflict and every candidate reports `mutation_allowed:false`. |
| Repair approval | operator-only `repair/approve` appends one `var1.repair_candidate_approval.v1` receipt bound to candidate sequence/id, patch hash, and expected/current baseline; approval evidence reports `mutation_allowed:true`, but no source mutation occurs. |
| Repair application | operator-only `repair/apply` verifies candidate/approval sequence, exact `replace_in_file` JSON payload/tag, resolved target, patch hash, and source baseline, then dispatches the existing reviewed write tool and appends `var1.repair_candidate_applied.v1`; retries reuse applied-receipt or committed-intent identity. |
| Repair replay | operator-only `repair/rerun` requires the immutable replay receipt plus a later applied receipt, creates one fresh `continued_from_session_id` treatment child, and routes the recorded input/model/provider/mode through `session/send`; `repair_rerun_started`/`repair_rerun_completed` relationship receipts and pre-dispatch input/config hash gates make identity and provider dispatch observable. |
| Repair evaluation | `repair/rerun` appends one idempotent `var1.repair_evaluation.v1` receipt to the source event spine; it compares baseline/treatment outcomes, turn latency, conservative observable tool-span side effects, token/cost evidence, exact identity/provider invariants, and explicit optional bounds. File-effect certainty remains `var1.tool_effect.v1`; evaluator mutation is forbidden. |
| Repair rollback | operator-only `repair/rollback` binds a failed evaluation to candidate/approval/applied sequence evidence, requires the current source baseline and full current-file SHA-256, then routes an exact inverse payload through the existing reviewed `replace_in_file` writer; `var1.repair_rollback.v1` started/completed receipts retain the failed treatment and hypothesis, and completion requires the candidate pre-apply hash. |
| Repair promotion/recovery | `repair/rerun` promotes only a passing treatment over a failed/cancelled baseline through one idempotent `var1.repair_regression.v1` receipt; `session/get`/`session/list` cold-start reconciliation closes orphaned rerun/rollback starts exactly once from the existing event spine and never repeats provider or file mutation. Ambiguous rollback bytes remain `recovery_required`. |
...
- `events.zig::FailureReceipt` is the only failure identity owner. Normalize class, phase, and bounded detail once; derive the deterministic `failure-<sha256>` ID; preserve it through session cold projection and scheduler ticket reconciliation; and reuse the same owner for stale-lease requeue. Do not add a repair ledger, diagnostic bus, or ticket-local failure hash.
- `core/evaluation/events.zig::appendRepairReceiptEvent` is the sole root-turn replay-boundary owner. Append one receipt to the existing event spine before context/provider work; retain exact input/model, hash transient config/tool/catalog/source/environment, and never persist raw snapshots or add a second repair ledger.
- `shared/protocol/events.zig::RepairDiagnosis` is the sole causal-diagnosis shape. Derive it from normalized failure evidence inside terminal serialization; keep invariant labels fixed, bind the span to stored sequence numbers, and do not create a diagnosis event, free-form telemetry field, or second ledger.
- `core/tools/builtin/repair_candidate.zig` is the sole proposal owner for Move 74. It resolves through `fsutil.resolveWithAccessMode`, requires read-ledger inspection and an existing target, validates the exact `replace_in_file` JSON payload/tag, captures the before hash, hashes the operation/path/patch descriptor, appends `var1.repair_candidate.v1` to the existing event spine, and returns `RepairBaselineConflict` on source drift. Every candidate reports `mutation_allowed:false`; later approval/application uses the existing reviewed write tools. Do not add a patcher, approval bus, or repair ledger.
- `stdio_rpc.zig::handleRepairCandidateApprove` and `evaluation/events.zig::appendRepairCandidateApprovalEvent` own Move 75 approval. `stdio_rpc.zig::handleRepairCandidateApply` and the adjacent evaluation event helpers own Move 76 application. `repair/approve` binds candidate event sequence/id, exact stored patch hash, and expected source baseline; `repair/apply` rechecks the approval, resolved target, exact patch, current baseline, and retry identity before dispatching the canonical reviewed `replace_in_file` tool. The write tool remains the sole source mutation owner and supplies inspection, stale-tag, effect, and write-intent evidence. Do not expose approval/application as model tools or add a broker, queue, patcher, or second ledger.
- `stdio_rpc.zig::handleRepairRerun` and `loop.zig::RunOptions.expected_replay_*` own Moves 77–78 treatment admission/evaluation. `stdio_rpc.zig::handleRepairRollback` owns Move 79 rollback: it validates the failed evaluation and candidate/approval/applied edge, requires current source and full-file hashes, and reuses the reviewed `replace_in_file` writer before appending a completed receipt only when the candidate pre-apply hash is restored. `evaluation/events.zig::appendRepairRegressionEvent` and `stdio_rpc.zig::reconcileRepairLifecycles` own Move 80 promotion and cold-start closure: promote only a passing treatment over a failed/cancelled baseline, append one `var1.repair_regression.v1` receipt, and reconcile orphaned control-plane starts without replaying provider or file mutation. Persist only compact relationship/evaluation/rollback/regression metrics and hashes. Do not substitute a prompt, copy the parent transcript, add a replay provider path, add an evaluator worker/ledger, use git reset/checkout, add a patcher, or turn an interrupted start into an implicit retry.
- `ask_user` is a root-only interactive tool. It emits one bounded `input_requested` event, normalizes options to `a`–`f` with `f = Other`, and resolves through `input/respond`; root normal, root-agent, and orchestrator-only catalogs retain it while child profiles remain headless and fail with `InputUnavailable` instead of hanging. The host `InputBroker` is process-local wake state only, keyed by session plus request id; session cancellation and shutdown broadcast to pending waits. Do not add a second question overlay, polling loop, resolved-event bus, or child-specific input system.
- `clients/question_view.zig::State` is the one question projection: render bounded batches as a full-frame settings-style modal with one horizontal row per visible question, clamped question/option focus, and an explicit review/submit state. The modal owns the Vaxis frame before transcript/footer layout, so long batches, reasoning rows, status-bar placement, and zero-height transcript geometry cannot collide with it. `orchestrate`, `build`, `align`, and `plan` share this controller; root normal is a catalog/profile name, not another prompt mode. Reject empty, wrong-schema, wrong-kind, or over-`input_protocol.max_serialized_bytes` requests before parse; Vaxis-borrowed question text must remain in State-owned, static, or frame-owned storage until `vx.render`; the display projection sanitizes invalid UTF-8/control text, truncates by terminal cell width, uses static display keys while preserving response ids, and guards clipped rows. Multi-select Enter/Space toggles stay on the question until review; malformed `input_requested` data is recoverable client input, so report it once and cancel the waiting run without unwinding the TUI event loop. Missing ownership, transport failure, terminal replay, and settled-turn boundaries clear stale input state; active Ctrl-C uses `input/respond` cancellation.
