...
- `execution-owner` and foreground `serve` acquire the same crash-released workspace lease and publish one atomic projection at `.var/runtime/execution-owner.json`; clean shutdown removes only the exact matching generation/PID/port/token/workspace projection, while crash-stale or replaced state remains for fail-closed diagnosis. Only `apps/backend/src/host/http_bridge.zig` may construct the private `ChildClient` from `stdio_client.zig`; do not add another daemon, pool, scheduler, or reconnect transport.
...
| Replay/repair extension | Not part of the kernel floor. Sessions, transcripts, terminal evidence, and reviewed write tools remain the canonical capability; any future repair loop requires a new owner-mapped YAGNI gate. |
...
- `core/prompts/builder.zig::PromptMode` owns the session-local `orchestrate -> build -> align -> plan` prompt lens and root orchestration posture. The session record owns the active label: an explicit `session/send` label resolves, validates, and persists it; an omitted label follows the stored record and only falls back to `orchestrate` for fresh sessions; unknown labels fail before session/provider execution. Shift+Tab is the fast TUI control. The `set_prompt_mode` root tool (present in all four modes, headless children get `ToolUnavailable`) lets the agent switch the mode itself: it validates the label, persists through the host `PromptModeService`, emits one `var1.prompt_mode_changed.v1` event with from/to and a bounded reason, and the executor loop reconciles its active lens plus `orchestrator_only` gating from the durable record after the executing tool batch — provider/model routing stays as resolved at send time. The TUI projects the event onto the footer label and one bounded system row. `orchestrate` exposes only agent/collaboration/interactive tools to a root run and requires the current eligibility snapshot for agent actions; `build`, `align`, and `plan` retain the normal root catalog. `agent_routes.prompt_modes` may select a provider/model and turn budget for one mode through the existing route shape; explicit `session/send` overrides win. Child capability profiles are unchanged, and no persistent `agents.orchestrator_only` policy exists.
...
- Repair/replay control plane is intentionally retired. Do not add a repair ledger, candidate broker, replay provider path, evaluator worker, patcher, rollback queue, or hidden retry. Reopen it only after a fresh owner map proves a concrete user capability that the existing session, event, ticket, and reviewed-write owners cannot provide.
- `ask_user` is a root-only interactive tool. It emits one bounded `input_requested` event, normalizes options to `a`–`f` with `f = Other`, and resolves through `input/respond`; every root prompt mode retains it while child profiles remain headless and fail with `InputUnavailable` instead of hanging. The host `InputBroker` is process-local wake state only, keyed by session plus request id; session cancellation and shutdown broadcast to pending waits. Do not add a second question overlay, polling loop, resolved-event bus, or child-specific input system.
- `clients/question_view.zig::State` is the one question projection: a full-frame modal showing exactly ONE question at a time with its full-context prompt (grapheme-wrapped, never truncated to a column budget) and its options stacked vertically, one option per row, with the option description on an indented dim row that drops first under height pressure. Ctrl+Q/Ctrl+E wrap-cycle questions (never entering the review state); Up/Down/Left/Right move the clamped option cursor; Tab advances without wrap and enters the explicit review/submit state from the last question. The modal owns the Vaxis frame before transcript/footer layout, so long batches, reasoning rows, status-bar placement, and zero-height transcript geometry cannot collide with it. `orchestrate`, `build`, `align`, and `plan` share this controller; root normal is a catalog/profile name, not another prompt mode. Reject empty, wrong-schema, wrong-kind, or over-`input_protocol.max_serialized_bytes` requests before parse; Vaxis-borrowed question text must remain in State-owned, static, or frame-owned storage until `vx.render`; the display projection sanitizes invalid UTF-8/control text, truncates by terminal cell width, uses static display keys while preserving response ids, and guards clipped rows. Multi-select Enter/Space toggles stay on the question until review; malformed `input_requested` data is recoverable client input, so report it once and cancel the waiting run without unwinding the TUI event loop. Missing ownership, transport failure, terminal replay, and settled-turn boundaries clear stale input state; active Ctrl-C uses `input/respond` cancellation.
- `clients/models_view.zig::State` is the model-selection overlay and `models_view.Catalog`/`fetchCatalog` is the one catalog cache behind the palette, picker, and Settings Models tab: one flat list from `models/list-all` (every credentialed provider, active group first, per-group unreachable degradation), grouped under provider headers with the active model marked. The composer palette is argument-aware: `model` alone lists the command, `model <prefix>` live-filters entries by case-insensitive prefix on model id OR provider id with Enter committing through `providers/set-model` using the model's source provider — a cross-provider pick selects the provider in the same action, the TUI never manages active-provider state itself. Bare `model` + Enter opens the full-frame picker; commit flips the footer model and appends one bounded system row; per-agent overrides remain the Settings → Models assign layer plus `agents/configure`, which opens on the flat catalog with a `P`-toggle providers mode for import. Do not add a second catalog owner, client-side provider switching, a poller, or a per-surface catalog fetch.
...
- Ticket assignment, scheduler claims, leases, live-owner heartbeat, generation-fenced same-session resume, absent-session requeue, and terminal reconciliation remain one typed queue-to-agent state machine; health fields are a read projection only.
...
6. Arena/quota discipline: keep the existing turn, provider, tool, and UI allocation owners bounded; do not add scoped-arena or quota infrastructure until measured pressure names one consumer and one readback proof.
...
15. Local performance telemetry: keep runtime health, typed turn usage, event sequence, and session-ledger evidence as the active low-noise observability owners; do not add a counter registry or stats command until a measured bottleneck, canonical owner, explicit operator consumer, and readback proof exist.
