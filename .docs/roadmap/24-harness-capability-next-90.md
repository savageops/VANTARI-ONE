...
| 74 | Generate a candidate patch receipt with baseline conflict detection before any mutation. | source-complete: the existing tool registry/runtime expose proposal-only `repair_candidate`; it requires an inspected existing target, captures the before hash, hashes the exact operation/path/patch descriptor, appends one `var1.repair_candidate.v1` event to `events.jsonl`, and returns typed `RepairBaselineConflict` when the expected source baseline differs. No write intent, approval, or mutation occurs. Debug/ReleaseFast `19/19`, `2,182/2,182`; source ReleaseFast `9/9`; source SHA-256 `E92BD7C72EBF06D2D6B43F0ECF85B90AD6E0C34605D72833B96CBD5F0B7BB0FD`; installed promotion deferred. Receipt: `.docs/todo/changelog/068-repair-candidate-baseline.md`. |
...
| 76 | Apply the approved patch in isolation through existing reviewed tools and write-intent evidence. | tools/sessions; no parallel patcher. |
