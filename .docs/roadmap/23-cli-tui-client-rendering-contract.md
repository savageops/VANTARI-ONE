...
- **TUI as read model** (`apps/backend/src/clients/tui_chat.zig`).
  - Hydrates from `session/get` (transcript + events) on load, then drains live `session/event` notifications by transport queue position while rendering only contiguous durable event sequences. Sequence-less legacy activity is ignored. A gap or completed turn requests `session/get { after_seq, events_only }`; the normal contiguous path performs no replay RPC.
  ...
  - **Replay identity is exact.** `last_event_seq` is the sole per-session render cursor. Equal timestamp/type/message payloads remain distinct when their stored sequences differ; an already consumed sequence cannot render twice. Child `assistant_response` rows use the canonical `summaries.jsonl` projection; the removed 512-entry content-key cache owns no fallback identity.
