...
This ledger converts the full-harness SITREP into executable work. The convergent root cause is process-local ownership presented as durable ownership: request workers, agent workers, scheduler leadership, summary writes, message sequence allocation, client replay, and test roots lose one serialized authority at a process or concurrency boundary.
