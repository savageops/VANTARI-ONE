...
- [x] `pricing.zig` is a pure module: const table + pure functions, no I/O, no state; provenance comments on every price row
- [x] `compat.zig` owns ALL detection truth — no URL-substring matching scattered in `openai_compatible.zig` or dispatch
- [x] `turn_payload.zig` is the single owner of turn boundary JSON — grep proves loop.zig and supervisor.zig contain no duplicate payload builders
- [x] `types.Usage` is a value type with no allocation/deinit burden on `CompletionResponse`
- [x] No file exceeds its natural ownership boundary; no parallel systems created (no new event type, no cost DB, no price service)

### 2. Contract truth (capability completion)
- [x] All 3 providers fill `CompletionResponse.usage` (non-stream + stream)
- [x] `turn_finished` events in `events.jsonl` carry `prompt_tokens/completion_tokens/cached_tokens/cost_total_usd` (null for unpriced models)
- [x] z.ai requests byte-identical to pre-chain (enable_thinking top-level preserved) — the primary operator lane cannot regress
- [x] DeepSeek gets nested thinking + reasoning_content forwarding; standard endpoints get neither
- [x] `wire_api: "auto"` validates everywhere (config floor, role overrides) and resolves per base_url in dispatch
- [x] `/status` shows session cost; TUI parses v2 telemetry

### 3. Test pressure (≥30 meaningful tests across the chain)
- [x] 035a usage parse: ≥12 blocks
- [x] 035b pricing: ≥10 blocks
- [x] 035c anthropic/responses: ≥8 blocks
- [x] 035d turn payload: ≥6 blocks
- [x] 035e compat: ≥12 blocks
- [x] 035f config: ≥3 blocks
- [x] 035g TUI: ≥4 blocks
- [x] Total ≥ 55 — exceeds the 30 floor; every block asserts an externally observable contract (parse output, payload JSON, request JSON, rendered status)

### 4. Code quality (anti-pattern sweep)
- [x] No hidden fallback readers or parallel state surfaces (AGENTS.md §XII)
- [x] Capability comments above every new/touched function (turn_payload, pricing, compat, extraction helpers)
- [x] No inline TODO left open; comment-and-TODO discipline per slice
- [x] JSONL/event readers unaffected (v2 payload is additive; ignore_unknown_fields proven)
- [x] `zig build test` fully green
- [x] No broad rewrites, no speculative abstraction (each new module has ≥2 real consumers or is the single canonical owner)

### 5. Pipeline integrity (entire chain, provider → event → client)
- [x] Response parse → `CompletionResponse.usage` → pricing → `turn_finished` v2 → events.jsonl → TUI telemetry → `/status` — each hop evidenced by a test
- [x] Config floor (`auto`) → dispatch resolution → request shape → provider — each hop evidenced
- [x] Manual proof recorded: one real kernel-stdio turn against z.ai shows v2 `turn_finished` in events.jsonl with measured tokens

## Completion Checklist

- [x] All defects found during review fixed (chain extended if any)
- [x] Evidence recorded in every unit (no PLACEHOLDER)
- [x] Chain archived: parent + all units in `/todo/changelog/`
- [x] `.docs/todo/changelog/_log.md` updated
- [x] Handoff cold-start ready from repository state
...
## Source and terminal QC verdict: PASS
...
- 035a usage parse, 035b pricing (incl. comptime table integrity), 035c anthropic/responses, 035d turn payload, 035e compat (detection + request-shape + dispatch routing), 035f config validation, 035g TUI telemetry — all in the current 1964 green graph. Plus 10 new agent-registry schema cases (doctrine/ticket/autonomy/effort/temperature round-trip + rejection) verified directly. The added 035c Responses-stream cache-default probe closes the stated eight-block adapter pressure floor.
...
- `zig build test` fully green: 19/19 steps, 1,964/1,964 tests.
...
Source structure, tests, and installed consumer proof pass. The installed
priced/unknown provider turns, event-ledger readback, multiline `/status`,
ReleaseFast hash equality, and final zero-process census are recorded in 035g.
035h is ready for archival with its parent.
