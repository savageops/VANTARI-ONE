 ...
 *
 * Handles direct communication with the Vantari bridge's `/v1/chat/completions` endpoint.
 ...
 *   - Handles HTTP requests/responses with the Vantari bridge
 ...
 *   - Returns typed `ApiVantariServerProps` responses
 *
 ...
 * - CORS proxy routing via Vantari bridge for cross-origin MCP servers
 ...
 *
 * **Current Migrations:**
 * 1. **Legacy message format**: Marker-based → Structured format
 * 2. **Theme key**: Copy standalone `theme` → config object (both preserved)
 *
 * **Usage:**
 * ```typescript
