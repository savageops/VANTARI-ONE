 ...
 *
 * **Current Migrations:**
 * 1. Legacy message format: Transform in-place (preserves structure, migrates markers)
 * 2. Theme key: Copy standalone `theme` → config object (both preserved)
 *
 * Pre-fork storage-name migrations were removed: the Vantari web app starts
 * from a fresh `Vantari` storage prefix with no legacy databases to adopt.
 */
...
// Migration 1: Legacy Message Format
...
// Migration 2: Theme Key (Non-Destructive)
