<script lang="ts">
	...
	import { HEADERS, MCP_SERVER_URL_PLACEHOLDER } from '$lib/constants';
...
<div class="grid gap-2">
	...
	{#if !isWebSocket && onUseProxyChange}
		...
		>
			...
			<span>
				<span class="text-xs text-muted-foreground">Use server proxy</span>

				<br />

			{#if !mcpStore.isProxyAvailable}
				<span class="inline-flex gap-0.75 text-xs text-muted-foreground/60"
					>(Server proxy is not available on this bridge)</span
				>
			{/if}
			</span>
		</label>
