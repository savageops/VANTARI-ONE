...
class ModelsStore {
	 ...
	 */
	private modelPropsCache = new TTLCache<string, ApiVantariServerProps>({
	...
	getModelProps(modelId: string): ApiVantariServerProps | null {
	 ...
	 */
	async fetchModelProps(modelId: string): Promise<ApiVantariServerProps | null> {
	...
	private buildModalities(
		modalities: NonNullable<ApiVantariServerProps['modalities']>
	): ModelModalities {
		return {
