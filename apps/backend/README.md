...
| Repair candidate | **Source-complete; installed promotion pending** | Proposal-only `repair_candidate` requires an inspected existing target, captures its before hash, hashes the exact operation/path/patch descriptor, appends `var1.repair_candidate.v1` with expected/current source baselines, and returns typed `RepairBaselineConflict` on drift. Ready candidates still report `mutation_allowed:false`; no write intent or mutation occurs. Debug/ReleaseFast `19/19` / `2,182/2,182`; source ReleaseFast `9/9`; installed promotion deferred. |
