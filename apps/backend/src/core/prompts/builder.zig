...
pub fn buildAgentSystemPromptWithMemoryAndMode(
    ...
    const agent_mode_note = if (execution_context.orchestrator_only)
        "Orchestrator-only mode is active. Direct task-artifact tools are unavailable in this parent context. Use agents {} when task evidence requires a specialist; the eligible snapshot lets you choose whether to inspect, message, challenge, launch, accept queueing, wake, or remain quiet. The snapshot omits private child instructions and transcripts. Launch only with explicit bounded context and a finite task. Use background:false to park immediately, or background:true only when independent orchestration work remains. The parent wakes on the first ready child result and resumes automatically."
    else
    ...
    try writer.print(
        ...
        \\
        \\## Delegation Protocol
        \\Call agents {{}} for the route-eligible specialist and current-team snapshot; list_agents is demand-loaded launched-run detail. The snapshot is evidence, not an order. Let the active system prompt and operator request choose whether collaboration helps and whether to stay quiet, inspect, message, challenge, launch, queue, or wake. The kernel enforces route availability, scope, fixed capacity, depth, and contact budgets.
        \\Launch useful branchable tasks from self-contained prompts — parallel external research, independent repository reconnaissance, isolated implementation, or validation. Keep the parent on routing, sequencing, convergence, and the final operator response. Read-only tracks (research, recon) can parallelize freely; write-heavy tracks serialize one-per-file-set. When parallel work is justified, emit the needed launch_agent calls in one assistant turn.
        ...
        \\Every session maintains a permanent <=100-word summary in the append-only summary ledger (.var/sessions/summaries.jsonl), readable by any session, subagent, or future run through session_summaries — the timeline of every session's last summary. You MUST call update_session_summary before your turn ends: capture the objective, key decisions, work completed, open threads, and next steps in under 100 words. Ending a turn without updating it violates the contract — a truncated fallback is written automatically, visible in the ledger as source:kernel_fallback. Subagents must update their own summary before returning their SITREP. Before delegating or continuing cross-session work, read session_summaries to recall what other sessions were doing, what they concluded, and what they left open.
