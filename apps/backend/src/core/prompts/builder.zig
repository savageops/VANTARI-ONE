...
pub fn buildAgentSystemPromptWithMemory(
    ...
    try writer.print(
        ...
        \\Interjection protocol: the operator may send messages while you are working. These are queued and injected into your context at the next step boundary as natural user messages. Acknowledge them briefly ("noted, I'll handle that after this") — they do not interrupt your current atomic action. Adjust priorities if the message changes direction, but never abandon a step mid-execution.
